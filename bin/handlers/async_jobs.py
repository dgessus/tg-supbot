...
async def notify_admin(update: Update, context: ContextTypes.DEFAULT_TYPE, notification_type: Literal['new_ticket', 'ticket_overdue'], ticket_id: int = None) -> None:
    logger.info(f'notifying admins about ticket', extra={"user_id" : update.effective_chat.id})
    ...
    try:
        ...
        logger.warning(f"failed to send ticket notification: {e}", extra={"user_id" : update.effective_chat.id})


async def clear_user_data(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await asyncio.sleep(300)
    context.user_data['user_messages'].clear()
    logger.info(f"auto-cleared messages from context.user_data for user", extra={"user_id" : update.effective_chat.id})
    return None
